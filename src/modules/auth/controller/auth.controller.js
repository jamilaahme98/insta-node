...
export const signup = async(req,res)=>{
    try{
        ...
        if(user){
           ...
           const saveUser = await userModel.create({userName:name1,Fullname:name2,email,password:hash});
           if(!saveUser){
   ...
   const {email,password} = req.body;
    ...
    if(!user){
      ...
      if(!user.confirmEmail){
        res.status(400).json({message:"plz confirmed email"});
      }
...
export const forgetPassword = async(req,res)=>{
  try{
    const {sendCode,email,newPassword} = req.body;  //i need this information 
    if(sendCode == null){
